...
description: Complete click-by-click setup for connecting Salesforce to Mari — pick scope, mint an OAuth access token + instance URL (Salesforce CLI, a Connected App, or an existing session), authenticate, and sync Knowledge articles and Cases. Use when the user wants to connect or add Salesforce as a Mari source.
version: 0.1.0
user-invocable: true
...
Run every Mari command yourself via Bash:

```
mari <cmd>
```

Work the four parts in order. Ask, wait for the answer, then act —
...
**Default to global.** You'll set this in Part 4 with `mari scope salesforce global|local`.

---

## Part 2 — How the connection works

Mari connects to Salesforce with two pieces of information:
...
Mari stores exactly the two values you hand it and verifies the token against the Salesforce REST
API at connect time.
...
less). Mari does not refresh them — it stores what you give it and nothing more. So expect this:
...
> `mari auth salesforce --token <new-token> --url <same-url>`. Nothing else changes; the index
> stays put.
...
10. Mint an access token against that app. The simplest route is still the CLI pointed at the app:
    ...
    response's `access_token` and `instance_url` are the two values Mari needs.
...
```
mari auth salesforce --token <access-token> --url https://YOURDOMAIN.my.salesforce.com
```
...
- **Write the credential file** — run `mari init search` to see the credential file path, then write
  `{"token": "<access-token>", "url": "https://YOURDOMAIN.my.salesforce.com"}` to it directly.

On success Mari verifies the token and prints `✓ Salesforce connected`.
...
```
mari status
mari search "<topic>" --source salesforce
```
...
Once connected, Mari indexes **both** Salesforce **Knowledge articles** and **Cases** via SOQL —
...
- `salesforce:articles` or `salesforce:cases` in the `objects` list only **narrows** the sync to
  one of the two. Leave it empty to get both. (Use `mari track add salesforce salesforce:articles` if
  the user explicitly wants Knowledge only, for example — `mari track add` asks whether the ref goes
  in your personal config or the team-shared committed config.)
...
and re-run the `mari auth salesforce …` line with the same `--url`.
