...
description: Complete click-by-click setup for connecting HubSpot to Mari — pick scope, create a private-app access token with CRM + Knowledge Base read scopes, authenticate, and sync tickets, CRM notes, and knowledge-base articles. Use when the user wants to connect or add HubSpot as a Mari source.
version: 0.1.0
user-invocable: true
...
# Connect HubSpot to Mari

Walk the user through connecting **HubSpot** end to end. Run every Mari command yourself via Bash:

```
mari <cmd>
```

Work the four steps in order. Steps 1–2 are quick questions; step 3 is a full walkthrough of the HubSpot UI you can read aloud to the user while they click; step 4 is the commands you run.
...
5. On the **Basic Info** tab, enter a **Name** — for example `mari knowledge search`. A description and logo are optional; leave them or fill them in.
6. Click the **Scopes** tab. This is a searchable checklist of permissions, each with **Read** and **Write** checkboxes. Mari only needs **Read**. Use the **search box** at the top to find each scope and tick its **Read** box:
   - **Tickets** — search `tickets` and enable **Read** on the support **Tickets** scope. Covers support tickets.
   - **Notes / engagements** — search `notes` (or `engagements`) and enable **Read** on the CRM notes/engagements scope. This is what lets Mari read CRM note bodies. If a distinct notes scope isn't offered in this account, enable **Read** on the broader **CRM** object read scope that covers contacts and engagements.
...
If the account **doesn't have the Knowledge Base product**, that's fine — you can skip that scope, and Mari tolerates a missing knowledge base and simply skips articles.
...
1. **Authenticate.** The command is:
   ...
   ```
   mari auth hubspot --token <private-app-token>
   ```
   ...
   - **Write the credential file** — run `mari init search` to print the credential file path, then write `{"token": "pat-…"}` to that path directly.

   On success Mari confirms the connected **portal** id.

2. **Set the scope** from step 1:

   ```
   mari scope hubspot global    # or: local
   ```
...
3. **Sync** to build the index:
   ...
   This indexes **all three collections — support tickets, CRM notes, and knowledge-base articles** — automatically, as soon as the source is connected. There's nothing to "track" or add first, and there's no first-sync lookback window: Mari re-observes the whole collection each run.

   Optionally, to **narrow** what's indexed to specific kinds, add one or more of `hubspot:tickets`, `hubspot:notes`, or `hubspot:kb`:

   ```
   mari track add hubspot hubspot:tickets    # index only tickets, for example
   ```

   `mari track add` asks whether the ref goes in your personal config or the team-shared committed config. These refs only *restrict* the set — omit them and everything syncs.
...
```
mari search "<topic>" --source hubspot
```
