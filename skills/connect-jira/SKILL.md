---
name: connect-jira
description: Complete click-by-click setup for connecting Jira to Mari — pick scope, choose Atlassian Cloud or Server/Data Center, walk through creating the exact credential, authenticate, track projects, and sync. Use when the user wants to connect or add Jira as a Mari source.
version: 0.1.0
user-invocable: true
...
# Connect Jira to Mari

Guide the user through connecting **Jira** end to end. You run every Mari command yourself via Bash:

```
mari <cmd>
```

Work the four steps in order. Keep it a back-and-forth: ask, wait for the answer, then act. Don't dump all four steps at once.

What gets indexed: for each project you track, Mari pulls every issue and stores **one document per issue** — its **summary**, **description**, and all **comments**. Search then hits issue text directly. You must track at least one project or a sync indexes nothing. There is no first-sync lookback window; the first sync reaches the full history of each tracked project, and later syncs are incremental (Mari keeps a per-project `jira.since.{PROJ}` cursor and only re-pulls issues updated since).
...
Remember the answer; you set it with `mari scope jira …` in step 4.

## 2. Connection method — Cloud or Server/Data Center?

Jira comes in two flavors and Mari supports both. Ask the user which they have — the fastest tell is the URL they use to open Jira in a browser:
...
Mari decides which flow to use purely by whether you pass `--email`: supply it and you get the Cloud (Basic) flow; omit it and you get the Server/DC (Bearer) flow. So getting this right matters.
...
3. Give it a **Label** (e.g. `mari`) so you can recognize it later.
4. Click **Create**.
...
5. Give it a **name** (e.g. `mari`) and set an **expiry** if prompted.
...
- **Cloud:**
  ```
  mari auth jira --url https://YOURSITE.atlassian.net --email you@company.com --token <api-token>
  ```
- **Server / Data Center:**
  ```
...
3. **They write the file** — run `mari init search`, read out the **credential file path** it prints for Jira, and have them write the JSON directly:
...
```
mari scope jira global
```
or
```
mari scope jira local
```

### 4c. Track at least one project

Mari only indexes projects you list. Add each by its **project key** (the uppercase prefix on issue keys — the `ENG` in `ENG-1234`).

Run `mari track add jira <ref>` once per project — it adds the project to the jira **`projects`** list, asking whether the ref goes in your personal config or the team-shared committed config. Each ref accepts either form:

- `jira:PROJ` (e.g. `jira:ENG`), or
- a browse URL like `https://YOURSITE.atlassian.net/browse/PROJ-123` (Mari pulls the project key out of it).

Run it once per project you want searchable. Removing a project later and re-syncing prunes that project's issues from the index.
