...
You are driving this setup for the user. Run every Mari command yourself via Bash:

```
mari <cmd>
```

Work the four parts in order. This is a longer walkthrough than Mari's other skills on purpose — go step by step, confirm each part before moving on, and don't skip the Slack-app screens.
...
4. Enter an **App Name** (e.g. `mari-search`) and pick the **workspace** you want to index under **Pick a workspace to develop your app in**.
...
That token is all Mari needs. Give it to the assistant (or keep it private — see Part 4).
...
Private channels require both `groups:read` **and** `groups:history`. If those scopes are missing, Mari quietly falls back to indexing **public channels only** (it logs `token lacks groups:read — indexing public channels only`) instead of failing. If you expect private-channel results and don't get them, re-check those two scopes and reinstall the app.
...
```
mari auth slack --token xoxp-…
```

Mari calls Slack's `auth.test` to validate the token and stores it, then prints `✓ Slack connected as <user> in <team>`.

**Offer the privacy path** — a token is a sensitive credential, so give the user three ways to provide it:
1. **You run it** — paste the token into the `mari auth slack --token …` command above (default).
2. **They run it** — hand the user the exact `mari auth slack --token …` line to run in their own terminal, so the token never passes through the chat.
3. **They write the file** — run `mari init search` to print the credential file path, and have the user write `{"token":"xoxp-…"}` there directly.
...
```
mari scope slack global
```

(or `mari scope slack local`).
...
```
mari config set slack.lookback_days <N>
```
...
```
mari search "<topic>" --source slack
```
...
```
mari track add slack "#channel-name"
```

`mari track add` asks whether the ref goes in your personal config or the team-shared committed config.

With no channels added, Mari syncs **all** channels the token can see — that's the intended default.
