...
description: Complete click-by-click setup for connecting Discord to Mari — pick scope, create and invite a Discord bot, get its token, track channels/guilds, and sync. Use when the user wants to connect or add Discord as a Mari source.
version: 0.1.0
user-invocable: true
...
# Connect Discord to Mari

Guide the user through connecting **Discord**, end to end. Run every Mari command yourself via Bash:

```
mari <cmd>
```

Work the four parts in order. This is a longer walkthrough than most Mari skills on purpose — Discord requires creating and inviting a bot, so don't skip the click-by-click detail. Read the exact steps to the user, wait where they need to act, and confirm before moving on.
...
Discord has **no user-token API** for reading message history the way Mari needs, so Mari uses a **bot** that you create and invite to the server(s) you want indexed. The bot reads message text through the Discord API using a **bot token**.
...
- Mari indexes **one document per message** in the channels/guilds you track that the bot can access. It does not index a whole server automatically — you list what to track in part 4.
...
3. Enter a name (e.g. `mari`) → check the terms box → click **Create**.
...
```
mari auth discord --token <bot-token>
```
...
- **Write the file** — run `mari init search` to get the credential file path, then write `{"token": "<bot-token>"}` to that path directly.

On success Mari prints `✓ Discord connected as <name>.`

### 4b. Set the scope

```
mari scope discord global|local
```
...
The bot indexes nothing until you tell it what to track. Run `mari track add discord <ref>` for each channel or guild — it adds the reference to the `channels` or `guilds` list, asking whether the ref goes in your personal config or the team-shared committed config. Accepted reference forms (as shown by `mari init search`):
...
- Only if they want something other than 14:
  ...
  ```
  mari config set discord.lookback_days <N>
  ```

After the first sync, Mari tracks a per-channel cursor and only pulls new messages, so this setting matters mainly for the initial backfill.
...
```
mari status
mari search "<topic>" --source discord
```
