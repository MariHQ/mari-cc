---
name: connect-gdocs
description: Complete click-by-click setup for connecting Google Drive / Docs to Mari — install the Google Cloud CLI, sign in through gcloud's browser flow (no token, nothing to configure in Google Cloud), set scope, and sync. Use when the user wants to connect or add Google Drive or Google Docs as a Mari source.
version: 0.1.0
user-invocable: true
...
# Connect Google Drive to Mari

Walk the user through connecting **Google Drive** (Google Docs + PDFs). You run every Mari command yourself via Bash:

```
mari <cmd>
```

Work the four parts in order and keep it a short back-and-forth — ask the scope question, get gcloud installed, run the sign-in, then sync.
...
- **Auth** uses the provider name **`google`**: `mari auth google`
- **Scope, config, sync, search** use the source key **`gdocs`**: `mari scope gdocs …`, `mari config set gdocs.…`, `mari sync gdocs`, `mari search "…" --source gdocs`
...
You'll set this in part 4 with `mari scope gdocs global|local`.
...
- **Nothing to set up in Google Cloud.** No GCP project, no OAuth consent screen, no API enablement. The `gcloud` CLI ships with Google's own pre-verified OAuth client, so signing in through gcloud is all the authorization Mari needs.
- **One prerequisite:** the **Google Cloud CLI** (`gcloud`) must be installed on this machine. That's part 3.

Under the hood: `mari auth google` runs `gcloud auth login --enable-gdrive-access` (opens the browser), and each sync mints a short-lived Drive-scoped token via `gcloud auth print-access-token`. You don't touch any of that — it's automatic once you're signed in.
...
You do **not** need to run `gcloud init` or `gcloud config` — Mari's auth step handles the sign-in. There is **no token to fetch**; the browser sign-in in part 4 is the credential.
...
1. **Sign in** (provider name is **google**):
   ...
   Because it's an interactive browser flow, you (the assistant) run the command and then tell the user to complete the prompt in their browser — the token never passes through you. A privacy-minded user can instead run `mari auth google` themselves in their own terminal. When it finishes, Mari prints `✓ Google connected`.

2. **Set scope** (source key is **gdocs**), using the choice from part 1:
   ```
   mari scope gdocs global
   ```
   (or `local`).
...
3. **First-sync lookback.** Ask: **"How many days of Drive history should the first sync backfill?"**
   ...
   - Only if they want something other than 30:
     ```
     mari config set gdocs.lookback_days <N>
     ```
...
4. **Optional — skip Drive comments.** By default Mari indexes each Drive comment as its own searchable entry (so "Eric's last comment on my launch doc" is answerable). To turn that off:
   ```
   mari config set gdocs.comments false
   ```
...
6. **Confirm:**
   ```
   mari status
   mari search "<a topic you know is in your docs>" --source gdocs
   ```
...
- Adding a specific **Google Doc or Drive folder URL** only **narrows** the scope — it's optional and turns off the auto-index-everything behavior:
  ```
  mari track add gdocs "<Google Doc or Drive folder URL>"
  ```
  `mari track add` asks whether the ref goes in your personal config or the team-shared committed config. Use this only if you want to index a specific shared doc/folder instead of your whole Drive.

## Known gotcha — sync fails on a blocked gcloud binary

If a sync fails because the sandbox blocks the `gcloud` binary (Mari can't mint a token), re-run the sync with the **tool sandbox disabled**. The auth itself is fine — it's only the per-sync `gcloud auth print-access-token` call that the sandbox intercepts.
