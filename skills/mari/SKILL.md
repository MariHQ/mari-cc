...
- **Connector setup** ("connect Slack", "add my Jira") → invoke the matching guided
  ...
  `connect-salesforce`, `connect-hubspot`, `connect-microsoft`, `connect-discord`,
  `connect-granola`) rather than improvising the steps.
...
`granola`, `localfiles`. `git` is the repo's commit history (no auth, indexed by default) — `search
