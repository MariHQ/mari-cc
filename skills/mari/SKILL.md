...
- `detect`, `audit`, `asset`, `platform`, `surface`, `check`, `docsite`, `i18n`, `rules`, `nudge`, `glossary`, `config`, `status`, and `features`: run the CLI command directly. Use `--strict` when the user asks for a release or CI gate.
...
- `mari docsite plan` and `mari docsite status` guide a repository-wide documentation pass.
...
Use `mari glossary list` to inspect the terminology table in `STYLE.md`. Add a preferred term with `mari glossary add <term> --use <preferred> --not <discouraged>` only after the user approves the terminology choice.
