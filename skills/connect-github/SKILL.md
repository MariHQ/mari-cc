---
name: connect-github
description: Complete click-by-click setup for connecting GitHub to Mari — pick scope, create a fine-grained or classic personal access token, authenticate, track repos, and sync issues and PRs. Use when the user wants to connect or add GitHub as a Mari source.
version: 0.1.0
user-invocable: true
...
Guide the user through connecting **GitHub** as a knowledge source. Mari indexes the **issues and pull requests** (title, body, and comments) of the repos you track, so you can search them locally.
...
Mari authenticates with a GitHub **personal access token (PAT)**. Any token that grants **read access to the target repos' issues and pull requests** works. There are two kinds of PAT — both are accepted by the same `mari auth github --token <token>` command:

- **(A) Fine-grained personal access token** *(recommended)* — scoped to specific repositories and specific read-only permissions. Least privilege: the token can only read what Mari needs. Prefix `github_pat_…`.
...
Note: GitHub has **no auto-indexing** — Mari only indexes repos you explicitly track. You must add at least one repo (step 4) or a sync does nothing. There is no first-sync lookback window; the first sync pulls all issues/PRs, then incremental syncs pull only what changed.
...
2. Under **Token name**, enter a name, e.g. `mari`.
3. Under **Expiration**, pick an expiry. Shorter is safer; you'll re-run `mari auth github` with a fresh token when it expires. (Optionally add a **Description**.)
...
5. Under **Repository access**, choose:
   - **Only select repositories** — then pick the exact repos you want Mari to index (recommended, least privilege), **or**
...
2. In the **Note** field, enter a name, e.g. `mari`.
...
3. **Track at least one repo.** Run `mari track add github <repo>` once per repo — it adds the repo to the github **`repos`** list, asking whether the ref goes in your personal config or the team-shared committed config. Each entry is a repo as **`owner/name`** (e.g. `octocat/Hello-World`) **or** a full **github.com URL** (e.g. `https://github.com/octocat/Hello-World`). Add every repo the user wants indexed. Removing a repo from this list later (`mari track remove github <repo>`) prunes everything indexed under it on the next sync.

4. **Build the index.**
   ```
   mari sync github
   ```
   This fetches every issue and pull request for each tracked repo — title, body, and comments — and embeds them. Mari keeps a per-repo `github.since` cursor, so later syncs only pull items that changed (nothing is re-fetched or re-embedded unnecessarily).
...
5. **Confirm.**
   ...
   then a test query:
   ```
   mari search "<topic>" --source github
   ```
