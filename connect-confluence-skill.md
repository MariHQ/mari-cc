...
description: Complete click-by-click setup for connecting Confluence to Mari — pick scope, choose Atlassian Cloud or Server/Data Center, create an API token or Personal Access Token, authenticate, track spaces and pages, and sync. Use when the user wants to connect or add Confluence as a Mari source.
version: 0.1.0
user-invocable: true
...
# Connect Confluence to Mari

Guide the user through connecting **Confluence** end to end. You run every Mari command yourself via Bash:

```
mari <cmd>
```

Work the four steps in order. This is a deliberately thorough walkthrough — read the exact button and field names to the user, confirm each credential before moving on, and don't skip steps.

Confluence comes in two flavors and Mari supports both:
...
Mari auto-detects which one from whether you pass `--email`: supply `--email` and it uses Cloud; omit it and it uses Server/Data Center.
...
3. In the **Label** field, enter a name you'll recognize later, e.g. `mari`.
4. Click **Create**.
...
5. Give it a **name** (e.g. `mari`) and set an **expiry** (or leave it non-expiring if the admin allows).
...
```
mari scope confluence global|local
```
...
Nothing syncs until you track content. Run `mari track add confluence <ref>` for each space or page — it asks whether the ref goes in your personal config or the team-shared committed config, then adds it to the right list:

- **Track a whole space** — pass the space key; it goes in the `spaces` list. Every page in the space is indexed.
- **Track individual pages** — pass page ids; they go in the `pages` list.

Each ref may be written as any of these accepted forms (as shown by `mari init search`):
...
```
mari status
mari search "<topic>" --source confluence
```
