...
description: Complete click-by-click setup for connecting Zendesk to Mari — pick scope, create a Zendesk API token in Admin Center, authenticate with subdomain + email + token, and sync tickets and help-center articles. Use when the user wants to connect or add Zendesk as a Mari source.
version: 0.1.0
user-invocable: true
...
# Connect Zendesk to Mari

This guide walks the user through connecting **Zendesk** to Mari end to end. You (the assistant) run
every Mari command yourself via Bash:

```
mari <cmd>
```

Work the four parts in order. Keep it a short back-and-forth: ask the
...
There is **one** way to connect Zendesk: a **Zendesk API token** used with HTTP Basic auth. Mari
...
Mari needs **three** pieces:

1. **Subdomain** — the `acme` in `acme.zendesk.com`. It's the first label of your Zendesk URL. (You
   can also hand Mari the full URL like `https://acme.zendesk.com` and it will extract `acme` for
   you.)
...
8. Optionally type an **API token description** (a label like `mari` so it's identifiable later).
...
```
mari auth zendesk --subdomain acme --email you@acme.com --token <api-token>
```

You may pass `--url https://acme.zendesk.com` instead of `--subdomain acme` — Mari extracts the
...
- **Write the credential file** — run `mari init search` to get the credential file path, then write
  `{"subdomain": "acme", "email": "you@acme.com", "token": "<api-token>"}` directly to that path.

On success Mari verifies the token against `/users/me` and prints `✓ Zendesk connected as <name>`.
...
```
mari scope zendesk global      # or: mari scope zendesk local
```
...
**Optional narrowing:** if the user wants only one kind, run `mari track add zendesk zendesk:tickets`
(or `zendesk:articles`) to add it to the `include` list — `mari track add` asks whether the ref goes
in your personal config or the team-shared committed config. This *narrows* the sync to that one kind — it is never required to enable
...
```
mari status
mari search "<topic>" --source zendesk
```
